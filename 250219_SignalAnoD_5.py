...
st.title("Global Laser Welding Anomaly Detection")
    ...
    if uploaded_file:
        ...
        for file in csv_files:
            ...
            for bead_num, (start, end) in enumerate(segments, start=1):
                ...
                all_segments.append((file, bead_num, features))
        
        feature_matrix = np.array([seg[2] for seg in all_segments])
        scaler = RobustScaler()
        ...
        results = [(seg[0], seg[1], pred, score) for seg, pred, score in zip(all_segments, predictions, anomaly_scores)]
        
        st.write("## Visualization")
        fig = go.Figure()
        for file, bead_num, pred, score in results:
            df = pd.read_csv(file)
            segment = df.iloc[start:end + 1]
            color = 'red' if pred == -1 else 'black'
            fig.add_trace(go.Scatter(y=segment[filter_column], mode='lines', line=dict(color=color, width=1),
                                     name=f"{file} - Bead {bead_num}",
                                     hoverinfo='text',
                                     text=f"File: {file}<br>Bead: {bead_num}<br>Status: {'Anomalous' if pred == -1 else 'Normal'}<br>Score: {score:.4f}"))
        fig.update_layout(title="Global Anomaly Detection Results", xaxis_title="Time Index", yaxis_title="Signal Value", showlegend=True)
        st.plotly_chart(fig)
    st.success("Global anomaly detection complete!")
