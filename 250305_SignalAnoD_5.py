...
if "metadata" in st.session_state:
    ...
    if selected_bead:
        ...
        for entry in st.session_state["metadata"]:
            if entry["bead_number"] == selected_bead:
                df = pd.read_csv(entry["file"])
                signal = df.iloc[:, 0].values
                status = st.session_state.get("anomaly_results_isoforest", {}).get((entry["file"], selected_bead), "normal")
                anomaly_score = st.session_state.get("anomaly_scores_isoforest", {}).get((entry["file"], selected_bead), 0)
                ...
                fig.add_trace(go.Scatter(
                    ...
                    name=f"{entry['file']}",
                    hoverinfo='text',
                    text=f"File: {entry['file']}<br>Status: {status}<br>Anomaly Score: {anomaly_score:.4f}"
                ))
